...
void GraphExecutor::Init(nnvm::Symbol symbol,
  ...
  sa_log << "Before initializing anything" << std::endl;
  ...
  // Load the dataflow graph generated by SwapAdvisor.
  bool export_graph = dmlc::GetEnv("MXNET_SWAP_EXPORT_GRAPH", 0);
  bool swap_advisor = dmlc::GetEnv("MXNET_SWAP_ADVISOR", 0);
  sa_log << "export_graph = " << export_graph << std::endl;
  sa_log << "swap_advisor = " << swap_advisor << std::endl;
  if (!export_graph) {
    if (swap_advisor) {
      auto _context = g.attrs["context"];
      auto _device = g.attrs["device"];
      sa_log << "Before SA_LoadGraph" << std::endl;
      ...
      #endif
      sa_log << "After SA_LoadGraph" << std::endl;
    }
  }
...
void GraphExecutor::FinishInitGraph(nnvm::Symbol symbol,
  ...
  g = AttachOpExecs(g);
  sa_log << "AttachOpResources" << std::endl;
  AttachOpResources(g);
  graph_ = std::move(g);

  sa_log << "Before InitDataEntryMemory" << std::endl;
  if (shared_exec != nullptr) {
    this->InitDataEntryMemory(&(dynamic_cast<GraphExecutor*>(shared_exec)->data_pool_));
  } else {
    this->InitDataEntryMemory(nullptr);
  }
  sa_log << "After InitDataEntryMemory" << std::endl;

  {
    sa_log << "Before initializing output arrays" << std::endl;
    ...
    sa_log << "Before initializing head gradient arrays" << std::endl;
    ...
    sa_log << "After initializing head gradient arrays" << std::endl;
  }
  sa_log << "Before InitCachedOps" << std::endl;
  this->InitCachedOps();
  sa_log << "After InitCachedOps" << std::endl;
  bool swap_advisor = dmlc::GetEnv("MXNET_SWAP_ADVISOR", 0);
  if (swap_advisor) {
    this->SaveEntryMapping();
  }
  this->ExportEntryHandle();
  sa_log << "Before InitOpSegs" << std::endl;
  this->InitOpSegs();
  sa_log << "After InitOpSegs" << std::endl;
}
...
void GraphExecutor::ExportEntryHandle() {
  sa_log << "ExportEntryHandle" << std::endl;
  const auto& idx = graph_.indexed_graph();
...
void GraphExecutor::SaveEntryMapping() {
  sa_log << "SaveEntryMapping" << std::endl;
  const auto& idx = graph_.indexed_graph();
  std::vector<std::vector<size_t>> output_handles;
...
Executor *Executor::Bind(nnvm::Symbol symbol,
                         ...
                         const std::vector<NDArray> &aux_states,
                         Executor* shared_exec) {
  sa_log << "Before binding anything" << std::endl;
  auto exec = new exec::GraphExecutor();
  exec->Init(symbol, default_ctx, group2ctx,
