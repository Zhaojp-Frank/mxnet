...
void TofuCopyFromTo(const nnvm::NodeAttrs& attrs,
  ...
  Engine::Get()->PushSync([from, ret, offsets, sizes](RunContext ctx) {
    ret.CheckAndAlloc();
    for (size_t i = 0; i < from.size(); ++i) {
      ...
      if (f.ctx().dev_id == ret.ctx().dev_id) {
        ...
        cudaMemcpyPeerAsync(ret.data().dptr_,
                            ...
                            f.data().dptr_,
                            f.ctx().dev_id,
                            sizes[i].Size() * mshadow::mshadow_sizeof(ret.data().type_flag_),
                        s->stream_);
      }
    }
  ...
  priority,
  PROFILER_MESSAGE("TofuFusedConvrt"));
}
...
.set_num_outputs(1)
.describe("Operator used for tensor scheme conversion.")
.set_attr<FCompute>("FCompute<gpu>", TofuFusedConvertNoComm);
