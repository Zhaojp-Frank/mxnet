...
mpirun $OPTIONS -output-filename log_conv_without_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=2 -x TOFU_TILING_TYPE=kcuts $CMD -f $HOST 

echo "Doing $NP $B $H with communication"
mpirun $OPTIONS -output-filename log_conv_with_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=0 -x TOFU_TILING_TYPE=kcuts $CMD -f $HOST

echo "Doing $NP $B $H dp without communication"
mpirun $OPTIONS -output-filename log_conv_dp_without_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=2 -x TOFU_TILING_TYPE=datapar $CMD -f $HOST

echo "Doing $NP $B $H dp with communication"
mpirun $OPTIONS -output-filename log_conv_dp_with_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=0 -x TOFU_TILING_TYPE=datapar $CMD -f $HOST
