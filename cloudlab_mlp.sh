...
then
	...
	mpirun $OPTIONS -output-filename log_mlp_single_${B} -x MXNET_P2PNET_DEBUG=0 $CMD --address=127.0.0.1 -i 0
fi

echo "Doing $NP $B $H without communication"
mpirun $OPTIONS -output-filename log_mlp_without_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=2 -x TOFU_TILING_TYPE=kcuts -x MXNET_P2PNET_USE_MPI_BARRIER=0 $CMD -f $HOST 

echo "Doing $NP $B $H with communication"
mpirun $OPTIONS -output-filename log_mlp_with_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=0 -x TOFU_TILING_TYPE=kcuts -x MXNET_P2PNET_USE_MPI_BARRIER=1 $CMD -f $HOST

echo "Doing $NP $B $H dp without communication"
mpirun $OPTIONS -output-filename log_mlp_dp_without_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=2 -x TOFU_TILING_TYPE=datapar  -x MXNET_P2PNET_USE_MPI_BARRIER=1 $CMD -f $HOST

echo "Doing $NP $B $H dp with communication"
mpirun $OPTIONS -output-filename log_mlp_dp_with_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=0 -x TOFU_TILING_TYPE=datapar -x MXNET_P2PNET_USE_MPI_BARRIER=1 $CMD -f $HOST

echo "Doing $NP $B $H modelpar without communication"
mpirun $OPTIONS -output-filename log_mlp_mp_without_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=2 -x TOFU_TILING_TYPE=modelpar -x MXNET_P2PNET_USE_MPI_BARRIER=0 $CMD -f $HOST

echo "Doing $NP $B $H modelpar with communication"
#mpirun $OPTIONS -output-filename log_mlp_mp_with_comm_${NP}_${B}_${H} -x MXNET_P2PNET_DEBUG=0 -x TOFU_TILING_TYPE=modelpar -x MXNET_P2PNET_USE_MPI_BARRIER=1 $CMD -f $HOST
